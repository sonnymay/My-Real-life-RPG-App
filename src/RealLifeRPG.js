...
import { Play, Pause, Plus, CheckCircle, ArrowRight, Trash2, X, Save, Clock } from 'lucide-react';
...
const RealLifeRPG = () => {
  ...
  const [totalTimeTrackedSeconds, setTotalTimeTrackedSeconds] = useState(savedData.totalTimeTrackedSeconds);
  const [dailyGoalMinutes, setDailyGoalMinutes] = useState(savedData.dailyGoalMinutes);
  const [dailyTrackedMinutes, setDailyTrackedMinutes] = useState(savedData.dailyTrackedMinutes);
  ...
  const updateJobBasedOnLevel = (currentLevel, selectedClass) => {
    ...
    const progression = jobProgressions[selectedClass] || jobProgressions.Swordsman;
  ...
  const dailyProgressPercentage = Math.min((dailyTrackedMinutes / dailyGoalMinutes) * 100, 100);
    ...
    <div className="font-sans p-4 max-w-md mx-auto bg-gray-100 rounded-lg shadow-md">
      ...
      <div className="bg-white p-4 rounded-lg mb-4 shadow">
          ...
          <div className="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button 
                ...
                className={`p-2 rounded-md text-xs text-center ${jobClass === 'Swordsman' ? 'bg-red-100 text-red-800 font-bold' : 'bg-gray-100 hover:bg-gray-200'}`}
              >
                Swordsman
              </button>
              <button 
                onClick={() => selectClass('Archer')}
                className={`p-2 rounded-md text-xs text-center ${jobClass === 'Archer' ? 'bg-green-100 text-green-800 font-bold' : 'bg-gray-100 hover:bg-gray-200'}`}
              >
                Archer
              </button>
              <button 
                onClick={() => selectClass('Mage')}
                className={`p-2 rounded-md text-xs text-center ${jobClass === 'Mage' ? 'bg-purple-100 text-purple-800 font-bold' : 'bg-gray-100 hover:bg-gray-200'}`}
              >
                Mage
              </button>
              <button 
                onClick={() => selectClass('Thief')}
                className={`p-2 rounded-md text-xs text-center ${jobClass === 'Thief' ? 'bg-gray-100 text-gray-800 font-bold' : 'bg-gray-100 hover:bg-gray-200'}`}
              >
                Thief
              </button>
              <button 
                onClick={() => selectClass('Acolyte')}
                className={`p-2 rounded-md text-xs text-center ${jobClass === 'Acolyte' ? 'bg-yellow-100 text-yellow-800 font-bold' : 'bg-gray-100 hover:bg-gray-200'}`}
              >
                Acolyte
              </button>
              <button 
                onClick={() => selectClass('Merchant')}
                className={`p-2 rounded-md text-xs text-center ${jobClass === 'Merchant' ? 'bg-amber-100 text-amber-800 font-bold' : 'bg-gray-100 hover:bg-gray-200'}`}
              >
                Merchant
              </button>
            </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between mb-1">
            ...
            <span className="text-sm font-medium">{exp}/{expToLevel} EXP</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className={`h-2 rounded-full ${colors.progress}`} style={{ width: `${expPercentage}%` }}></div>
          </div>
        </div>
        
        <div className="mb-2">
          <div className="flex justify-between mb-1">
            <span className="text-sm font-medium">Next Job: {nextJobInfo.job}</span>
            <span className="text-sm font-medium">Level {level}/{nextJobInfo.level}</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className={`h-2 rounded-full ${colors.next}`} style={{ width: `${jobProgressPercentage}%` }}></div>
          </div>
        </div>
        ...
        <div className="text-xs text-gray-500 mt-3 pt-2 border-t border-gray-100">
          <div className="flex justify-between items-center">
            ...
            <span>{jobClass === 'Swordsman' ? 'Knight' : 
                   jobClass === 'Archer' ? 'Hunter' :
                   jobClass === 'Mage' ? 'Wizard' :
                   jobClass === 'Thief' ? 'Assassin' :
                   jobClass === 'Acolyte' ? 'Priest' :
                   jobClass === 'Merchant' ? 'Blacksmith' : ''}</span>
            <span>{jobClass === 'Swordsman' ? 'Lord Knight' : 
                   jobClass === 'Archer' ? 'Sniper' :
                   jobClass === 'Mage' ? 'High Wizard' :
                   jobClass === 'Thief' ? 'Guillotine Cross' :
                   jobClass === 'Acolyte' ? 'High Priest' :
                   jobClass === 'Merchant' ? 'Whitesmith' : ''}</span>
            <span>{jobClass === 'Swordsman' ? 'Rune Knight' : 
                   jobClass === 'Archer' ? 'Falcon Ranger' :
                   jobClass === 'Mage' ? 'Archmage' :
                   jobClass === 'Thief' ? 'Shadow Reaper' :
                   jobClass === 'Acolyte' ? 'Divine Cleric' :
                   jobClass === 'Merchant' ? 'Titan Forgemaster' : ''}</span>
            <span>{jobClass === 'Swordsman' ? 'Eternal Knight' : 
                   jobClass === 'Archer' ? 'Eternal Marksman' :
                   jobClass === 'Mage' ? 'Eternal Sorcerer' :
                   jobClass === 'Thief' ? 'Eternal Phantom' :
                   jobClass === 'Acolyte' ? 'Eternal Saint' :
                   jobClass === 'Merchant' ? 'Eternal Tycoon' : ''}</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-1 mt-1">
            <div className={`h-1 rounded-l-full bg-gray-500`} style={{ width: `${(level / 100) * 100}%` }}></div>
          </div>
      ...
      <div className="w-full h-48 bg-gradient-to-b from-orange-200 via-red-200 to-purple-300 rounded-lg mb-4 relative overflow-hidden">
        ...
        <div className="absolute bottom-16 left-1/4 w-32 h-32 flex items-center justify-center">
          ...
          <div className={`w-24 h-40 rounded-lg ${colors.character}`}></div>
        </div>
        
        {/* Class Icon */}
        <div className="absolute top-2 right-2 bg-white bg-opacity-70 p-2 rounded-full">
          {jobClass === 'Swordsman' && <div className="w-8 h-8 bg-red-500 rounded-full flex items-center justify-center text-white font-bold">S</div>}
          {jobClass === 'Archer' && <div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center text-white font-bold">A</div>}
          {jobClass === 'Mage' && <div className="w-8 h-8 bg-purple-500 rounded-full flex items-center justify-center text-white font-bold">M</div>}
          {jobClass === 'Thief' && <div className="w-8 h-8 bg-gray-500 rounded-full flex items-center justify-center text-white font-bold">T</div>}
          {jobClass === 'Acolyte' && <div className="w-8 h-8 bg-yellow-500 rounded-full flex items-center justify-center text-white font-bold">A</div>}
          {jobClass === 'Merchant' && <div className="w-8 h-8 bg-amber-500 rounded-full flex items-center justify-center text-white font-bold">M</div>}
        </div>
      ...
      <div className="bg-white p-4 rounded-lg mb-4 shadow border-2 border-green-300">
        ...
        <div className="flex items-center justify-center space-x-4">
          <div className="text-center">
            ...
            <div className="text-sm text-gray-500 font-bold">{levelsGained} levels gained today</div>
          ...
          <button 
            onClick={toggleTimer}
            className={`p-4 rounded-full ${isTimerRunning ? 'bg-red-100 text-red-500' : 'bg-green-500 text-white'}`}
          >
        ...
        <div className="mt-4 pt-3 border-t border-gray-100">
          <div className="flex justify-between text-xs text-gray-500 mb-1">
            ...
            <span>{Math.floor(dailyTrackedMinutes / 60)}h {dailyTrackedMinutes % 60}m / {Math.floor(dailyGoalMinutes / 60)}h goal ({Math.floor(dailyTrackedMinutes / 5)} levels)</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-green-500 h-2 rounded-full" style={{ width: `${dailyProgressPercentage}%` }}></div>
          </div>
          <div className="flex justify-between text-xs text-gray-500 mt-3">
            <div>Level Progress: 
              <span className="font-medium text-green-600"> 5 mins = 1 level</span>
            </div>
            <div>Current pace: 
              <span className="font-medium text-green-600"> 12 levels/hour</span>
            </div>
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg max-w-sm w-full mx-4">
              ...
              <p className="text-gray-600 mb-4">
                This will reset your timer to 00:00:00. Your progress for today will be lost. Are you sure?
              </p>
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="flex justify-between items-center mb-3">
          <div>
            ...
            <div className="text-xs text-gray-500 mt-1">
              {taskList.filter(task => task.completed).length}/{taskList.length} completed
            </div>
          </div>
          <button 
          ...
          >
            {showAddTask ? <X size={16} className="mr-1" /> : <Plus size={16} className="mr-1" />}
        ...
        <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded-md text-xs text-yellow-700">
          <strong>Note:</strong> Tasks don't affect leveling. Only running the timer counts toward levels (5 mins = 1 level).
        </div>
        ...
        <ul className="space-y-2">
          ...
          {[...taskList]
            ...
            <li 
              key={task.id} 
              className={`p-3 rounded-lg flex items-center justify-between ${task.completed ? 'bg-gray-100' : 'bg-blue-50'}`}
            >
              ...
              <div className="flex space-x-1">
                <button 
                  ...
                  className={`p-1 rounded-full ${task.completed ? 'text-green-500' : 'text-gray-400'} hover:bg-gray-100`}
                  title="Mark as complete"
                >
                  <CheckCircle size={20} className={task.completed ? 'fill-green-500' : ''} />
                </button>
                <button 
